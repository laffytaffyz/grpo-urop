...
### ~~PICK DATA~~
data_path = "/om/user/tiffany8/grpo-urop/TinyZero/dataset/mctaco_test_modified.parquet"

assert len(model_paths) == len(model_names)

class MCTACODataset(torch.utils.data.Dataset):
    def __init__(self, parquet_path, tokenizer, max_len=512):
        import pandas as pd, pyarrow.parquet as pq
        df = pq.read_table(parquet_path).to_pandas()
        self.questions = df["prompt"].str[0].str["content"].tolist()
        self.answers   = df["reward_model"].str["ground_truth"].tolist()
        self.tok = tokenizer
...
def main():
    ...
    for i in range(len(model_paths)):
        ...
        val_dataset   = MCTACODataset(data_path, tokenizer)
        ...
        with torch.no_grad():
            for prompt_texts, enc_batch, answers in val_dataloader:
                ...
                if data_counter >= 150 and total > 0: break
    ...
    print(df)
    df.to_json("/om/user/tiffany8/grpo-urop/TinyZero/response_analysis/mctaco/responses_mctaco.jsonl", orient="records", lines=True)
